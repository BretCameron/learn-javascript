...
class SignUpPage extends Component {
  ...
  updateInput = (e) => {
    ...
    switch (e.currentTarget.id) {
      case 'confirmPassword':
        password === e.currentTarget.value && e.currentTarget.value.length >= 1 ? this.setState({ passwordValid: true }) : this.setState({ passwordValid: false });
  ...
  handleSubmit = e => {
    e.preventDefault();
    const { firstName, lastName, email, isEmail, password, passwordValid, newsletter } = this.state;
    if (isEmail && passwordValid) {
      const newUser = { firstName, lastName, email, password, newsletter };
          ...
          <form type="POST">
            <div style={{
              ...
              <div>
                ...
                <input onChange={this.updateInput} id="firstName" name="first-name" type="text" value={firstName} />
              </div>
              <div style={{ gridColumn: '2 / 4', width: '100%' }}>
                <label htmlFor="last-name">Last Name</label>
                <input onChange={this.updateInput} id="lastName" name="last-name" type="text" value={lastName} />
              </div>
              <div style={{ gridColumn: '1 / 4', width: '100%' }}>
                <label htmlFor="email">Email Address</label>
                <input onChange={this.updateInput} id="email" name="email" type="email" value={email} />
              </div>
              <div>
                <label htmlFor="password">Password</label>
                <input onChange={this.updateInput} id="password" name="password" type={showPassword ? "text" : "password"} value={password} />
              </div>
              <div>
                <label htmlFor="password">Confirm Password</label>
                <input onChange={this.updateInput} id="confirmPassword" name="password" type={showPassword ? "text" : "password"} className={password === confirmPassword && confirmPassword !== '' ? 'passwords-match' : ''} value={confirmPassword} />
              </div>
              ...
              <input onClick={this.handleSubmit} type="submit" value="Sign Up" />
            </div>
            <p style={{fontStyle: 'italic', fontSize: '0.9rem'}}>{this.props.auth.message}</p>
          </form>
        </div>
      </div>
    )
